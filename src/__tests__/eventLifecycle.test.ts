...
describe("eventLifecycle", () => {
  it("expone únicamente las transiciones permitidas por estado", () => {
    ...
    expect(getEventLifecycleActions("publicado")).toEqual(["pause", "cancel"]);
    expect(getEventLifecycleActions("pausado")).toEqual(["publish", "cancel"]);
  ...
  it("traduce el avance incompleto a secciones accionables", () => {
    expect(getIncompleteEventSteps([true, false, true, false, true])).toEqual([
      "Fecha y ubicación",
      "Organización",
    ]);
  });

  it("adquiere el bloqueo una sola vez hasta liberar la solicitud", () => {
    const lock = { current: false };

    expect(acquireEventActionLock(lock)).toBe(true);
    expect(acquireEventActionLock(lock)).toBe(false);
    lock.current = false;
    expect(acquireEventActionLock(lock)).toBe(true);
  });
});
