...
export default function CrearCuentaInvitadoFlow({
  ...
  const [emailCuenta, setEmailCuenta] = useState(emailProp || '');
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!nombreProp || !apellidoPaternoProp) {
          try {
            const resDatos = await axios.get(`${API_URL}/users/phone/${telefono}`);
            setDatosPersona({
              nombre: resDatos.data.nombre || '',
              apellidoPaterno: resDatos.data.apellido_paterno || '',
              apellidoMaterno: resDatos.data.apellido_materno,
            });
            if (!emailProp && resDatos.data.email) {
              setEmailCuenta(resDatos.data.email);
            }
          } catch {
            // Sigue sin datos precargados si no se encuentra
          }
        }
  ...
  const handleCrearCuenta = async () => {
    ...
    try {
      await axios.post(`${API_URL}/auth/register`, {
        ...
        apellido_paterno: datosPersona.apellidoPaterno,
        apellido_materno: datosPersona.apellidoMaterno || undefined,
        telefono,
      });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: petzen.colors.background, justifyContent: 'center', alignItems: 'center', padding: 24 }}>
        ...
        {paso === 'prompt' && (
          <>
            <Text style={{ fontFamily: petzen.fonts.extraBold, fontSize: 24, color: petzen.colors.textDark, textAlign: 'center', marginBottom: 12 }}>
              {datosPersona.nombre ? `¡Hola, ${datosPersona.nombre}!` : '¿Quieres crear tu cuenta?'}
            </Text>
            <Text style={{ fontFamily: petzen.fonts.regular, fontSize: 15, color: petzen.colors.textSecondary, textAlign: 'center', lineHeight: 22, marginBottom: 32 }}>
