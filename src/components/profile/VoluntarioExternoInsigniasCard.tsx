...
const ROL = 'voluntario_externo';

// Comparte carpeta con voluntario_interno (assets/insignias/voluntarios/) --
// hogar_protector y relevo_seguro son .jpeg, el resto .png.
...
const IMAGENES_FIJAS: Record<string, any> = {
  casa_segura: require('../../../assets/insignias/voluntarios/casa_segura.png'),
  hogar_protector: require('../../../assets/insignias/voluntarios/hogar_protector.jpeg'),
  relevo_seguro: require('../../../assets/insignias/voluntarios/relevo_seguro.jpeg'),
};

const INFO: Record<string, { nombre: string; descripcion: string }> = {
  casa_segura: {
    nombre: 'Casa segura',
    descripcion: 'Tu hogar temporal fue aprobado formalmente',
  },
  hogar_protector: {
    nombre: 'Hogar protector',
    descripcion: '3 custodias temporales finalizadas',
  },
  relevo_seguro: {
    nombre: 'Relevo seguro',
    descripcion: '3 transferencias de custodia confirmadas (entrega o recepción)',
  },
  rescatista_pawalert: {
    nombre: 'Rescatista PawAlert',
    descripcion: 'Cobre: 1 resguardo · Plata: 5 resguardos · Oro: 15 resguardos',
  },
};
...
function formatProgreso(insignia: InsigniaReputacion): string {
  if (insignia.codigo_insignia === 'rescatista_pawalert') {
    return `${insignia.progreso} resguardo${insignia.progreso === 1 ? '' : 's'} exitoso${insignia.progreso === 1 ? '' : 's'}`;
  }
  if (insignia.codigo_insignia === 'hogar_protector') {
    return `${insignia.progreso} custodia${insignia.progreso === 1 ? '' : 's'} finalizada${insignia.progreso === 1 ? '' : 's'}`;
  }
  if (insignia.codigo_insignia === 'relevo_seguro') {
    return `${insignia.progreso} transferencia${insignia.progreso === 1 ? '' : 's'} confirmada${insignia.progreso === 1 ? '' : 's'}`;
  }
...
export function VoluntarioExternoInsigniasCard() {
  ...
  const pendientes = Object.keys(INFO).filter((codigo) => !codigosObtenidos.has(codigo));
  ...
  return (
    <View style={styles.card}>
      ...
      {isLoading ? (
        ...
        <Text style={styles.vacio}>Todavía no tienes insignias. Realiza tu primera custodia temporal para comenzar a ganar.</Text>
      ) : (
