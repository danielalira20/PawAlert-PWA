-- Respeta el eje "rescates" en el selector operativo de asociación.
--
-- Con la 0103 (participa_rescates / participa_adopciones) una asociación
-- puede apagar el eje de rescates sin dejar de existir como asociación
-- verificada. Sin este cambio, encontrar_asociacion_operativa seguiría
-- asignándole reportes nuevos igual que antes -- el filtro se agrega
-- junto a los otros gates operativos (activo, verificado,
-- recepcion_reportes_activa) para que una asociación que apagó rescates
-- nunca salga elegida, sin importar qué tan cerca o disponible esté.
--
-- No afecta reportes ya asignados a esa asociación (eso lo decide el
-- flujo de casos activos, no este selector), ni el eje de adopciones,
-- que no pasa por esta función.
...
) IS 'Selecciona coordinadora por cobertura, horario, capacidad, carga, distancia y eje de rescates activo.';
